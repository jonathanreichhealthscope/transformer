...
public:
    ...
    Matrix forward(const Matrix& input, bool training) const {
        ...
        if (input.rows() != dropout_mask.rows() || input.cols() != dropout_mask.cols()) {
            throw std::runtime_error("Dropout mask dimensions (" + 
                std::to_string(dropout_mask.rows()) + "," + std::to_string(dropout_mask.cols()) + 
    ...
    Matrix backward(const Matrix& grad_output) const {
        if (!mask_initialized) {
            throw std::runtime_error("Dropout mask not initialized. Forward pass must be called before backward pass");
        }
        
        if (grad_output.rows() != dropout_mask.rows() || grad_output.cols() != dropout_mask.cols()) {
            throw std::runtime_error("Gradient dimensions (" + 
                std::to_string(grad_output.rows()) + "," + std::to_string(grad_output.cols()) + 
