...
class SAM {
  ...
  public:
    SAM(float rho_ = 0.05f, std::unique_ptr<Optimizer> optimizer = nullptr) : rho(rho_) {
        if (!optimizer) {
    ...
    void compute_parameter_gradients(const Matrix& logits, const Matrix& target_distribution,
    ...
    void second_step(std::vector<Matrix*>& params, const std::vector<Matrix>& grads);
    void zero_grad() {
        base_optimizer->zero_grad();
    }
    void update_bias(std::vector<std::reference_wrapper<FloatVector>>& biases,
                     const std::vector<FloatVector>& bias_grads, float learning_rate = 0.001f);
    const Matrix& get_current_gradients() const {
