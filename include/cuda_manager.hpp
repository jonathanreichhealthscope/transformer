...
public:
  ...
  ~CudaManager() { CUDA_CHECK(cudaDeviceReset()); }
...
private:
  ...
  Matrix process_batch(const std::vector<Matrix> &inputs, size_t start,
                       size_t end) {
...
public:
  ...
  std::vector<Matrix> parallel_forward(const std::vector<Matrix> &inputs) {
    size_t batch_per_gpu =
        (inputs.size() + num_gpus - 1) / num_gpus; // Ceiling division
    ...
    for (int i = 0; i < num_gpus; i++) {
      ...
      futures.push_back(
          std::async(std::launch::async, [this, &inputs, i, start, end]() {
