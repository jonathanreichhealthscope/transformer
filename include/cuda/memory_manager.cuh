...
public:
  CudaMemoryPool() { CUDA_CHECK(cudaStreamCreate(&stream)); }
...
public:
  CudaGraphManager() { CUDA_CHECK(cudaStreamCreate(&stream)); }

  template <typename F>
  void capture_and_execute(size_t key, F &&kernel_launch) {
