...
__device__ void warp_reduce_minmax(volatile float *smin, volatile float *smax,
                                   int tid) {
...
__global__ void find_minmax_kernel(const float *input, size_t size,
...
void find_minmax_cuda(const float *input, size_t size, float *min_val,
                      float *max_val) {
...
__global__ void quantize_kernel(const float *input, float *output, size_t size,
...
__global__ void dequantize_kernel(const float *input, float *output,
                                  size_t size, float scale, float zero_point) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
    float val = input[idx];
