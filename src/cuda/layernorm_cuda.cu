...
Matrix LayerNorm::forward_cuda(const Matrix &x) {
  ...
  // Copy data to device
  cudaMemcpy(d_input, x.data(), batch_size * hidden_size * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_gamma, gamma.data(), hidden_size * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_beta, beta.data(), hidden_size * sizeof(float),
             cudaMemcpyHostToDevice);
  ...
  layer_norm_stats_kernel<<<grid_size, block_size,
                            2 * block_size * sizeof(float)>>>(
      d_input, d_mean, d_variance, hidden_size, batch_size);

  const int norm_grid_size =
      (batch_size * hidden_size + block_size - 1) / block_size;
  layer_norm_kernel<<<norm_grid_size, block_size>>>(
      d_input, d_mean, d_variance, d_gamma, d_beta, d_output, hidden_size,
      batch_size, eps);

  // Copy result back to host
  cudaMemcpy(output.data(), d_output, batch_size * hidden_size * sizeof(float),
