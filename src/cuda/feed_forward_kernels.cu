...
__global__ void feed_forward_backward_kernel_1(
  ...
  if (idx < batch_size * intermediate_size) {
    ...
    for (int i = 0; i < hidden_size; ++i) {
      sum +=
          grad[batch_idx * hidden_size + i] * w2[inter_idx * hidden_size + i];
    }
    d_intermediate[idx] = sum;
  }
}

__global__ void gelu_backward_kernel(const float *d_intermediate,
                                     float *d_input, const int num_elements) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < num_elements) {
    float x = d_input[idx];
    float cdf = 0.5f * (1.0f + tanhf(0.797884f * (x + 0.044715f * x * x * x)));
    float pdf =
        0.797884f * (1.0f - tanhf(0.797884f * x) * tanhf(0.797884f * x));
