...
FeedForward::FeedForward(size_t hidden_size, size_t intermediate_size, float dropout)
    ...
    for (size_t i = 0; i < b1.size(); ++i) b1[i] = 0.0f;
...
Matrix FeedForward::forward(const Matrix& x) {
    ...
    for (size_t i = 0; i < x.rows(); ++i) {
        for (size_t j = 0; j < w1.cols(); ++j) {
            ...
            // GELU activation
            intermediate(i, j) = 0.5f * sum * (1.0f + std::tanh(
                std::sqrt(2.0f / M_PI) * (sum + 0.044715f * std::pow(sum, 3))
            ));
        }
    }
...
std::unique_ptr<FeedForward> FeedForward::load(std::istream& is) {
    ...
    is.read(reinterpret_cast<char*>(ffn->w1.data()), ffn->w1.rows() * ffn->w1.cols() * sizeof(float));
    is.read(reinterpret_cast<char*>(ffn->w2.data()), ffn->w2.rows() * ffn->w2.cols() * sizeof(float));
...
Matrix FeedForward::backward_cuda(const Matrix& grad, const Matrix& input) const {
    ...
    CUDA_LAUNCH(gelu_backward_kernel, grid_size, block_size, 0, 0,
