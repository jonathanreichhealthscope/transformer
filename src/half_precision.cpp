...
void HalfPrecisionTraining::convert_to_fp16(Matrix& matrix) {
    ...
    // Copy result back
    CUDA_CHECK(
        cudaMemcpy(half_data.data(), d_half, size * sizeof(half_type), cudaMemcpyDeviceToHost));
...
void HalfPrecisionTraining::convert_to_fp32(Matrix& matrix) {
    ...
    CUDA_CHECK(cudaMalloc(&d_half, size * sizeof(half_type)));
    CUDA_CHECK(
        cudaMemcpy(d_half, half_data.data(), size * sizeof(half_type), cudaMemcpyHostToDevice));
