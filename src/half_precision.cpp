...
void HalfPrecisionTraining::convert_to_fp16(Matrix &matrix) {
  ...
  // Copy input to GPU
  CUDA_CHECK(cudaMemcpy(d_float, matrix.data(), size * sizeof(float),
  ...
  // Copy result back
  CUDA_CHECK(cudaMemcpy(half_data.data(), d_half, size * sizeof(half_type),
...
void HalfPrecisionTraining::convert_to_fp32(Matrix &matrix) {
  ...
  CUDA_CHECK(cudaMalloc(&d_float, size * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_half, size * sizeof(half_type)));
  CUDA_CHECK(cudaMemcpy(d_half, half_data.data(), size * sizeof(half_type),
                        cudaMemcpyHostToDevice));

  // Launch conversion
  launch_fp16_to_fp32(d_half, d_float, size);

  CUDA_CHECK(cudaMemcpy(matrix.data(), d_float, size * sizeof(float),
                        cudaMemcpyDeviceToHost));
