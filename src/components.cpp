...
Matrix::Matrix(size_t rows, size_t cols, float init_val) {
  ...
  if (rows > SIZE_MAX / cols) {
    throw std::runtime_error(
        "Matrix dimensions too large - would cause overflow");
  }
  ...
  if (total_size > 1000000000) { // 1 billion elements max
    throw std::runtime_error(
        "Matrix dimensions too large - exceeds maximum allowed size");
  }
...
Matrix::Matrix(size_t rows, size_t cols, float *external_data)
    : data_(external_data, external_data + rows * cols), rows_(rows),
      cols_(cols), shape_(std::make_tuple(rows, cols)), owns_data_(false) {}

Matrix::Matrix(size_t rows, size_t cols, float *external_data, bool is_owner)
    : data_(external_data, external_data + rows * cols), rows_(rows),
      cols_(cols), shape_(std::make_tuple(rows, cols)), owns_data_(is_owner) {}
...
Matrix &Matrix::operator=(const Matrix &other) {
  if (this != &other) {
    ...
    try {
      ...
      throw std::runtime_error("Failed to assign matrix: " +
                               std::string(e.what()));
    }
  }
  return *this;
...
void Matrix::resize(size_t new_rows, size_t new_cols) {
  ...
  try {
    ...
    throw std::runtime_error("Failed to resize matrix: " +
                             std::string(e.what()));
  }
}
...
void Matrix::apply_gelu_derivative(const Matrix &x) {
  ...
  if (size() != x.size()) {
    throw std::runtime_error(
        "Matrix dimensions must match for GELU derivative");
  }
...
#pragma omp parallel for simd
  for (size_t i = 0; i < size(); i++) {
    ...
    float derivative =
        cdf + val * pdf * (1.0f - std::tanh(val) * std::tanh(val));
