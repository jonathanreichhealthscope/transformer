...
bool ModelSaver::saveCheckpoint(const Transformer& transformer,
    ...
    try {
        ...
        // Save model state
        if (!saveModel(transformer, directory, model_name + "_checkpoint_" + std::to_string(epoch))) {
            return false;
        }
...
bool ModelSaver::loadLatestCheckpoint(Transformer& transformer,
    ...
    try {
        ...
        for (const auto& entry : fs::directory_iterator(directory)) {
            if (entry.path().extension() == ".meta.json") {
                ...
                if (filename.find(model_name + "_checkpoint_") == 0) {
                    int checkpoint_epoch = std::stoi(filename.substr(filename.find_last_of("_") + 1));
        ...
        // Load model state
        return loadModel(transformer, directory, model_name + "_checkpoint_" + std::to_string(latest_epoch));
    } catch (const std::exception& e) {
