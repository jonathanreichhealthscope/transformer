...
float BeamSearch::apply_length_penalty(float score, size_t length) const {
    ...
    std::cout << "Applied length penalty: original_score=" << std::fixed << std::setprecision(4) 
              << score << ", length=" << length << ", penalized_score=" << penalized_score << std::endl;
...
std::vector<BeamSearch::Hypothesis> BeamSearch::search(
    ...
    for (size_t i = 0; i < beam_width_; ++i) {
        std::cout << "Token " << initial_topk[i].second 
                 << " (score: " << std::fixed << std::setprecision(4) 
                 << initial_topk[i].first << ")" << std::endl;
